...
from torch.nn import Sequential, Linear, ReLU, Tanh, Sigmoid, PReLU, GRUCell, Embedding, Module, Parameter, LayerNorm
...
class Encoder(torch.nn.Module):
    def __init__(self, num_features, dim, num_gc_layers, summary_iter=2):
        ...
        self.graph_mu_norm = LayerNorm(dim)
        self.graph_logvar_norm = LayerNorm(dim)
    ...
    def forward(self, x, edge_index, batch):
        ...
        graph_latent_space_mu = self.graph_mu_norm(self.graph_mu_proj(slots))
        graph_latent_space_logvar = self.graph_logvar_norm(self.graph_logvar_proj(slots))
...
class Encoder_base(torch.nn.Module):
    ...
    def forward_gnn_directout(self, x, edge_index):
        for i in range(self.num_gc_layers):
            x = F.relu(self.convs[i](x, edge_index))
            #x = self.bns[i](x)
